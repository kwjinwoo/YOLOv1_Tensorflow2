...
class DatasetLoader:
    ...
    def point_adjust(self, points, h, w):
        x = float(self.img_size / w) * points[..., 0]
        y = float(self.img_size / h) * points[..., 1]
        w = float(self.img_size / w) * points[..., 2]
        h = float(self.img_size / h) * points[..., 3]
        return tf.stack([x, y, w, h], axis=-1)

    def resize_and_scaling(self, img, name, bndboxes):
        img_shape = tf.shape(img)
        img = tf.image.resize(img, (self.img_size, self.img_size))
        adjust_bndboxes = self.point_adjust(tf.cast(bndboxes, dtype=tf.float32), img_shape[0], img_shape[1])
